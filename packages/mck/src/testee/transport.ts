...
//
// Shared JSON-lines transport for compatibility adapters. Callers issue one
// request at a time and validate response bodies for their suite. The
// transport assigns increasing ids and stops after a ProtocolError.
import { type ChildProcess, spawn } from "node:child_process";
import { readLines } from "../lines.ts";
import { ProtocolError, parseEnvelope } from "./protocol.ts";
...
export interface JsonLineClient {
	exchange(body: object): Promise<unknown>;
	close(): Promise<void>;
}

export function jsonLineClient(command: readonly string[], options: ProcessOptions): JsonLineClient {
	...
	// readable message this client reports.
	...
	// client can turn into a ProtocolError.
	...
	async function exchange(body: object): Promise<unknown> {
