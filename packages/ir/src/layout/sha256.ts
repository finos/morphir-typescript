...
//
// A pure TypeScript SHA-256 (FIPS 180-4) over UTF-8 bytes. The truncation
// rule in stems.ts needs a digest of the untruncated escaped stem, and needs
// it identically in Node and in a browser, so this avoids both `node:crypto`
// and `crypto.subtle` (the latter is async, and callers here are not).
...
function pad(bytes: Uint8Array): Uint8Array {
	...
	// 1 byte for 0x80, then zeros so the total is 56 mod 64, then an 8-byte
