...
export function mapType<TA, TB>(t: Type<TA>, f: (a: TA) => TB): Type<TB> {
	switch (t.kind) {
		case "Variable":
			return { kind: "Variable", attributes: f(t.attributes), name: t.name };
		case "Reference":
			return { kind: "Reference", attributes: f(t.attributes), fqname: t.fqname, args: t.args.map((a) => mapType(a, f)) };
		case "Tuple":
			return { kind: "Tuple", attributes: f(t.attributes), elements: t.elements.map((e) => mapType(e, f)) };
		case "Record":
			return { kind: "Record", attributes: f(t.attributes), fields: t.fields.map((x) => ({ name: x.name, type: mapType(x.type, f) })) };
		case "ExtensibleRecord":
			return {
				kind: "ExtensibleRecord",
				attributes: f(t.attributes),
				variable: t.variable,
				fields: t.fields.map((x) => ({ name: x.name, type: mapType(x.type, f) })),
			};
		case "Function":
			return { kind: "Function", attributes: f(t.attributes), parameterType: mapType(t.parameterType, f), returnType: mapType(t.returnType, f) };
		case "Unit":
			return { kind: "Unit", attributes: f(t.attributes) };
		default: {
			const _: never = t;
			return _;
		}
...
export function mapPattern<VA, VB>(p: Pattern<VA>, f: (a: VA) => VB): Pattern<VB> {
	switch (p.kind) {
		case "WildcardPattern":
			return { kind: p.kind, attributes: f(p.attributes) };
		case "AsPattern":
			return { kind: p.kind, attributes: f(p.attributes), pattern: mapPattern(p.pattern, f), name: p.name };
		case "TuplePattern":
			return { kind: p.kind, attributes: f(p.attributes), patterns: p.patterns.map((x) => mapPattern(x, f)) };
		case "ConstructorPattern":
			return { kind: p.kind, attributes: f(p.attributes), fqname: p.fqname, patterns: p.patterns.map((x) => mapPattern(x, f)) };
		case "EmptyListPattern":
			return { kind: p.kind, attributes: f(p.attributes) };
		case "HeadTailPattern":
			return { kind: p.kind, attributes: f(p.attributes), head: mapPattern(p.head, f), tail: mapPattern(p.tail, f) };
		case "LiteralPattern":
			return { kind: p.kind, attributes: f(p.attributes), literal: p.literal };
		case "UnitPattern":
			return { kind: p.kind, attributes: f(p.attributes) };
		default: {
			const _: never = p;
...
export function mapValue<TA, VA, TB, VB>(v: Value<TA, VA>, m: AttributeMapper<TA, VA, TB, VB>): Value<TB, VB> {
	...
	switch (v.kind) {
		case "Literal":
			return { kind: v.kind, attributes: va, literal: v.literal };
		case "Constructor":
			return { kind: v.kind, attributes: va, fqname: v.fqname };
		case "Tuple":
			return { kind: v.kind, attributes: va, elements: v.elements.map(val) };
		case "List":
			return { kind: v.kind, attributes: va, items: v.items.map(val) };
		case "Record":
			return { kind: v.kind, attributes: va, fields: v.fields.map((x) => ({ name: x.name, value: val(x.value) })) };
		case "Variable":
			return { kind: v.kind, attributes: va, name: v.name };
		case "Reference":
			return { kind: v.kind, attributes: va, fqname: v.fqname };
		case "Field":
			return { kind: v.kind, attributes: va, target: val(v.target), name: v.name };
		case "FieldFunction":
			return { kind: v.kind, attributes: va, name: v.name };
		case "Apply":
			return { kind: v.kind, attributes: va, function: val(v.function), argument: val(v.argument) };
		case "Lambda":
			return { kind: v.kind, attributes: va, pattern: mapPattern(v.pattern, m.onValue), body: val(v.body) };
		case "LetDefinition":
			return { kind: v.kind, attributes: va, name: v.name, definition: def(v.definition), in: val(v.in) };
		case "LetRecursion":
			return { kind: v.kind, attributes: va, definitions: v.definitions.map((b) => ({ name: b.name, definition: def(b.definition) })), in: val(v.in) };
		case "Destructure":
			return { kind: v.kind, attributes: va, pattern: mapPattern(v.pattern, m.onValue), value: val(v.value), in: val(v.in) };
		case "IfThenElse":
			return { kind: v.kind, attributes: va, condition: val(v.condition), then: val(v.then), else: val(v.else) };
		case "PatternMatch":
			return {
				kind: v.kind,
				attributes: va,
				value: val(v.value),
				cases: v.cases.map((c) => ({ pattern: mapPattern(c.pattern, m.onValue), body: val(c.body) })),
			};
		case "UpdateRecord":
			return { kind: v.kind, attributes: va, target: val(v.target), fields: v.fields.map((x) => ({ name: x.name, value: val(x.value) })) };
		case "Unit":
			return { kind: v.kind, attributes: va };
		case "Hole":
			return { kind: v.kind, attributes: va, reason: v.reason, expectedType: v.expectedType === null ? null : mapType(v.expectedType, m.onType) };
		default: {
			const _: never = v;
			return _;
		}
...
export function mapValueDefinition<TA, VA, TB, VB>(d: ValueDefinition<TA, VA>, m: AttributeMapper<TA, VA, TB, VB>): ValueDefinition<TB, VB> {
	...
	switch (d.kind) {
		case "ExpressionBody":
			return { kind: d.kind, inputTypes: mapInputs(d.inputTypes, t), outputType: mapType(d.outputType, t), body: mapValue(d.body, m) };
		case "NativeBody":
			return { kind: d.kind, inputTypes: mapInputs(d.inputTypes, t), outputType: mapType(d.outputType, t), nativeInfo: d.nativeInfo };
		// The bindings are plain strings, so they are copied; only the fallback
		// body carries attributes to rewrite.
		case "ExternalBody":
			return {
				kind: d.kind,
				inputTypes: mapInputs(d.inputTypes, t),
				outputType: mapType(d.outputType, t),
				externals: d.externals,
				body: d.body === null ? null : mapValue(d.body, m),
			};
		case "IncompleteBody":
			return {
				kind: d.kind,
				inputTypes: mapInputs(d.inputTypes, t),
				outputType: d.outputType === null ? null : mapType(d.outputType, t),
				incompleteness: mapIncompleteness(d.incompleteness, t),
				partialBody: d.partialBody === null ? null : mapValue(d.partialBody, m),
			};
		default: {
			const _: never = d;
			return _;
	...
	return as.map((a) =>
		a.kind === "Compact"
			...
			: {
					...
					name: a.name,
					args: a.args.map((x) =>
						x.kind === "Positional" ? { kind: "Positional", value: mapValue(x.value, m) } : { kind: "Named", name: x.name, value: mapValue(x.value, m) },
					),
				},
	);
}
...
export function mapTypeSpecification<TA, VA, TB, VB>(s: TypeSpecification<TA, VA>, m: AttributeMapper<TA, VA, TB, VB>): TypeSpecification<TB, VB> {
	...
	switch (s.kind) {
		case "TypeAliasSpecification":
			return { kind: s.kind, annotations, typeParams: s.typeParams, typeExp: mapType(s.typeExp, t) };
		case "OpaqueTypeSpecification":
			return { kind: s.kind, annotations, typeParams: s.typeParams };
		case "CustomTypeSpecification":
			return { kind: s.kind, annotations, typeParams: s.typeParams, constructors: mapConstructors(s.constructors, t) };
		case "DerivedTypeSpecification":
			return { kind: s.kind, annotations, typeParams: s.typeParams, baseType: mapType(s.baseType, t), fromBaseType: s.fromBaseType, toBaseType: s.toBaseType };
		default: {
			const _: never = s;
			return _;
		}
...
export function mapTypeDefinition<TA, TB>(d: TypeDefinition<TA>, f: (a: TA) => TB): TypeDefinition<TB> {
	switch (d.kind) {
		case "TypeAliasDefinition":
			return { kind: d.kind, typeParams: d.typeParams, typeExp: mapType(d.typeExp, f) };
		case "CustomTypeDefinition":
			return { kind: d.kind, typeParams: d.typeParams, constructorsAccess: d.constructorsAccess, constructors: mapConstructors(d.constructors, f) };
		case "IncompleteTypeDefinition":
			return {
				kind: d.kind,
				typeParams: d.typeParams,
				incompleteness: mapIncompleteness(d.incompleteness, f),
				partialTypeExp: d.partialTypeExp === null ? null : mapType(d.partialTypeExp, f),
			};
		default: {
			const _: never = d;
			return _;
...
export function mapModuleSpecification<TA, VA, TB, VB>(s: ModuleSpecification<TA, VA>, m: AttributeMapper<TA, VA, TB, VB>): ModuleSpecification<TB, VB> {
	return {
		annotations: mapAnnotations(s.annotations, m),
		doc: s.doc,
	...
	switch (d.kind) {
		case "Library":
			return {
				...
				dependencies: d.dependencies.map((x) => ({ name: x.name, value: mapPackageSpecification(x.value, m) })),
				def: mapPackageDefinition(d.def, m),
			};
		case "Specs":
			return {
				kind: d.kind,
				packageName: d.packageName,
				dependencies: d.dependencies.map((x) => ({ name: x.name, value: mapPackageSpecification(x.value, m) })),
				spec: mapPackageSpecification(d.spec, m),
			};
		case "Application":
			return {
				kind: d.kind,
				packageName: d.packageName,
				dependencies: d.dependencies.map((x) => ({ name: x.name, value: mapPackageDefinition(x.value, m) })),
				def: mapPackageDefinition(d.def, m),
