...
const name = (s: string): Name => {
	const r = Name.parse(s);
	if (!r.ok) throw new Error(s);
	return r.value;
};
const fq = (s: string): FQName => {
	const r = FQName.parse(s);
	if (!r.ok) throw new Error(s);
	return r.value;
};
...
test("mapValue rewrites nested type and value attributes", () => {
	...
	const v: Value<string, string> = {
		...
		then: { kind: "Hole", attributes: "v", reason: { kind: "UnresolvedReference", target: fq("a/b:c#d") }, expectedType: int },
			...
			kind: "Lambda",
			attributes: "v",
			pattern: { kind: "AsPattern", attributes: "v", pattern: { kind: "WildcardPattern", attributes: "v" }, name: name("x") },
