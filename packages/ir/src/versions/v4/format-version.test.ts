...
describe("recognize", () => {
	...
	test("rejects prerelease, leading zeros, majors below 3 as strings, and zero", () => {
		for (const bad of ["4.0.0-beta", "4.01.0", "2.0.0", "0.0.0"]) expect(recognize(root, bad).ok).toBe(false);
		expect(recognize(root, jsonNumber("0")).ok).toBe(false);
		expect(recognize(root, jsonNumber("4.5")).ok).toBe(false);
		expect(recognize(root, true).ok).toBe(false);
	});
	test("out of range", () => {
		const r = recognize(root, "3.4294967296.0");
...
describe.skipIf(corpus === null)("format-version-conformance.json", () => {
	...
	test("rootDiagnosticCases for JSON sources", () => {
		for (const c of corpus.rootDiagnosticCases.filter((x: { format: string }) => x.format === "json")) {
			...
			if (isObject(o)) {
				const r = readFormatVersionMember(root, o);
